    ...
    switch (handleReply.type) {
      case "reply": {
        ...
        if (s.length == 0) {
          ...
          for (let ad of idad) {
            api.sendMessage({
              body: "[=== CALLAD ===]\n❯ 📄Phản hồi từ " + name + ":\n" + (event.body) || "", attachment: l, mentions: [{
                id: event.senderID,
                tag: name
              }]
            }, ad, (e, data) => global.client.handleReply.push({
      ...
      case "calladmin": {
        if (s.length == 0) {
          ...
          api.sendMessage({ body: `✉ Phản hồi từ admin ${name} đến bạn:\n◆━━━━━━━━━━━━━━━━━◆\n${(event.body) || "『chỉ có ảnh/vd/voice/gif』"}`, attachment: l, mentions: [{ tag: name, id: event.senderID }] }, handleReply.id, (e, data) => global.client.handleReply.push({
            name: this.config.name,
            author: event.senderID,
            messageID: data.messageID,
            type: "reply"
